...
def create_user(infos):
    ...
    for info in infos:
        wrong_info = is_validation(info)
        if wrong_info == "blocked":
            wrong_user +=1
        elif wrong_info[0] == False:
            # is_wrong = wrong_info[0]

            wrong_point = wrong_info[1]
            wrong_user +=1
            for wrong in wrong_point:
                info[wrong] = None
            new_userlist.append(info)
        else :
            new_userlist.append(info)
           
    
    print(f"잘못된 데이터로 구성된 유저의 수는 {wrong_user}명 입니다.")
    pprint.pprint(new_userlist)        
        
def is_validation(info) :
    blood_ = info["blood_group"]
    company_ = info["company"]
    mail_ = info["mail"]
    name_= info["name"]
    website_ = info["website"]
    count_ = 0
    wrong_data = []
    if company_ in black_list:
        return "blocked"
    if blood_ not in blood_types:
        wrong_data.append("blood_group")
        count_ += 1
    if "@" not in mail_:
        wrong_data.append("mail")
        count_ += 1
    if not 2 <= len(name_) <= 30:
