...
falsedata_count = 0     # 잘못된 데이터로 구성된 유저 수 count
user_list = []      # 에러값 None으로 수정한 리스트
def create_user(data):
    for user in data:
        is_validation(user)
    print(f'잘못된 데이터로 구성된 유저의 수는 {falsedata_count}입니다. ')


def is_validation(x):
    global falsedata_count
    false_count = 0     # 잘못된 데이터가 있는 key 수
    false_info = []     # False, [잘못된 데이터가 있는 key]
    false = []      # 잘못된 데이터가 있는 key
    if not x['blood_group'] in blood_types:     # 혈액형 없음
        false_count += 1
        false.append('blood_group')
        x['blood_group'] = 'None'

    if x['company'] in black_list:      # 블랙 컴퍼니
        false_count += 1
    
    if not '@' in x['mail']:    # 메일에 @ 없음
        false_count += 1
        false.append('mail')
        x['mail'] = 'None'

    if not 2 <= len(x['name']) <= 30:   # 이름 짧거나 김
        false_count += 1
        false.append('name')
        x['name'] = 'None'

    if not 1 <= len(x['website']):  # 주소 0개
        false_count += 1
        false.append('website')
        x['website'] = 'None'

    if not x['company'] in black_list:
        user_list.append(x)


    if false_count >= 1:
        false_info.append('False')
        if len(false) != 0:
            false_info.append(false)
        falsedata_count += 1    # 잘못된 데이터가 있는 유저 카운팅
        return false_info       # False 정보 값 반환
    
    if false_count == 0:
        return True     # 잘못된 데이터 없는 경우
