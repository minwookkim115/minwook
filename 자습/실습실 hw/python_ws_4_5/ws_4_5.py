...
def create_user(data):
    ...
    for user in data:
        err_user = is_validation(user) # err_user는 is_validation(user)
        if err_user == "blocked":
            err_user_num += 1
        elif err_user[0] == False:
            err_user_num += 1

            err_val = err_user[1]
            for err in err_val:
                user[err] = None # => 이게 딕셔너리를 그냥 바꾸나
            user_list.append(user)
        else:
            user_list.append(user)

    print(f"잘못된 데이터로 구성된 유저의 수는 {err_user_num}명 입니다.")
    print(user_list)
...
def is_validation(user):
    ...
    if user["blood_group"] not in blood_types:
        err_list.append("blood_group")
        err_count += 1

    if "@" not in user["mail"]:
        err_list.append("mail")
        err_count += 1
    ...
    if user["website"] == False:
        err_list.append("website")
        err_count += 1
    
    if err_count >= 1:
        return False, err_list # 에러가 있으면 False에 err_list 반환
    
    else:
        return True, 1 # True일때 err_list에 안들어 가니까 1을 넣어서 위에 err_val = err_user[1]을 확인가능
